...
async def register_candidate(
    user_data: CandidateRegistrationRequest, db: Session = Depends(get_db)
):
    ...
    db_user.profile_completion_percentage = (
        calculate_candidate_profile_completion(db_user)
    )
    db.commit()
...
async def register_employer(
    user_data: EmployerRegistrationRequest, db: Session = Depends(get_db)
):
    ...
    # Calculate initial profile completion
    db_user.profile_completion_percentage = (
        calculate_employer_profile_completion(db_user)
    )
    db.commit()
...
async def register_admin(
    user_data: AdminRegistrationRequest, db: Session = Depends(get_db)
):
...
@router.post("/logout")
async def logout():
    """Logout user (token blacklisting would
    be implemented here in production)"""
...
async def update_advanced_candidate_profile(
    ...
    for field, value in update_data.items():
        if value is not None:
            ...
            if field in [
                ...
                # Handle JSON array fields - accept both list and string
                # formats
                if isinstance(value, str):
                    if value.strip() == "[]" or value.strip() == "":
                        setattr(current_user, field, [])
    ...
    current_user.profile_completion_percentage = (
        calculate_candidate_profile_completion(current_user)
    )
...
async def update_advanced_employer_profile(
    ...
    for field, value in update_data.items():
        if value is not None:
            # Handle type conversions for integer fields
            if field in ["founded_year", "employee_count"]:
                try:
                    if isinstance(value, str) and value.strip():
                        setattr(current_user, field, int(value))
                    elif isinstance(value, (int, float)):
    ...
    # Update profile completion percentage
    current_user.profile_completion_percentage = (
        calculate_employer_profile_completion(current_user)
    )
...
async def get_profile_completion(
    ...
    if current_user.user_type == ModelUserType.CANDIDATE:
        completion_data = calculate_candidate_profile_completion(
            current_user, return_details=True
        )
    elif current_user.user_type == ModelUserType.EMPLOYER:
        completion_data = calculate_employer_profile_completion(
            current_user, return_details=True
        )
    else:
        completion_data = {
...
def calculate_candidate_profile_completion(
    user: User, return_details: bool = False
):
    ...
    if getattr(user, "professional_title") and getattr(
...
def calculate_employer_profile_completion(
    user: User, return_details: bool = False
):
